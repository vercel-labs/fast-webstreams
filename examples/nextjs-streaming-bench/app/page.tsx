    ...
    <section className="mt-10 space-y-6">
      ...
      <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-6 space-y-4">
        ...
        <div className="flex gap-2 flex-wrap">
          {([
            ["SSR Default", "n=200&rows=20&groups=50&delay=1", "Default Next.js SSR pattern"],
            ["Stream Stress", "n=500&rows=0&groups=500&delay=0", "Max flushes, minimal React work — isolates streaming overhead"],
            ["Many Chunks", "n=500&rows=1&groups=500&delay=0", "500 separate flushes with tiny components"],
            ["Realistic SSR", "n=100&rows=10&groups=100&delay=0", "100 boundaries, staggered, small tables"],
          ] as const).map(([label, qs, desc]) => (
            <button
              key={label}
              title={desc}
              onClick={() => {
                setNativeUrl(`/bench-native?${qs}`);
                setFastUrl(`/bench-fast?${qs}`);
              }}
              className="px-3 py-1.5 text-xs rounded-lg bg-zinc-800 border border-zinc-700 hover:border-zinc-500 text-zinc-300 transition"
            >
              {label}
            </button>
          ))}
